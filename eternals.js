...
function findCharacters() {
    const attributeInput = document.querySelector('#attributeInput').value; // Directly take the input as is
    const resultsDiv = document.querySelector('#results');
    resultsDiv.innerHTML = ''; 

    eternals.forEach(character => {
        const foundSkill = character.skills.find(skill => skill === attributeInput); // Compare directly
        if (foundSkill) {
